@section('title', 'Bionix Roadshow Table')
<div class="flex flex-col items-center w-[95%] mx-auto p-4 min-h-screen oveflow-hidden font-montserrat">
    <h1 class="self-start mb-10 text-4xl font-bold text-transparent sm:text-5xl bg-gradient-green-r bg-clip-text">Data
        Pendaftar Bionix Roadshow</h1>
    <a href=" {{ route('admin.bionixroadshow.export') }} "
        class="px-6 py-2 mb-4 mr-auto bg-gradient-green-r rounded-xl hover:brightness-75">Export</a>
    <a href=" {{ route('admin.moodle') }} "
        class="px-6 py-2 mb-4 mr-auto bg-gradient-green-r rounded-xl hover:brightness-75">Export Moodle</a>
    <div
                ...
                @foreach ($pesertas as $peserta)
                    <tr class="font-medium text-lg h-16 border-b-[1px] border-white">
                        ...
                        <td class="pr-4 font-semibold text-center"><a
                                ...
                                href=" /admin/bionix-rd/{{ $peserta->ketua->id }}">Action</a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
