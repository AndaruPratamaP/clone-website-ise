...
<nav class="z-[9999] flex flex-row justify-between font-montserrat px-5 sm:px-[34px] items-center xl:px-[130px] pt-4 lg:pt-[50px] md:items-center fixed top-0 w-full"
    ...
    <!-- hamburger -->
    <button @click="isOpen = true" id="hamburger" name="hamburger" type="button" class="block md:hidden space-y-[2.77px]"
        :class="{ 'transition transform-180': isOpen }">
        <x-heroicon-o-bars-3 class="w-7 h-7 text-[#6B6F75]" />
    </button>

    <!-- navbar menu mobile-->
    <div x-show="{isOpen}" x-transition x-cloak @mousedown.outside="isOpen = false"
        ...
        <div class="flex flex-col gap-6">
            ...
            <a href="/"
                class="text-[0.875rem] sm:text-base bg-clip-text bg-gradient-to-r from-[#006C56] to-[#afd196] hover:text-transparent focus:text-transparent font-semibold">Home</a>
        ...
        <div class="flex flex-col gap-[7.72px] md:hidden text-[1rem] mt-2 pb-16">
            @if (Auth::check())
                <a href="{{ Auth::user()->isAdmin() ? route('admin.') : route('my.') }}"
                    class="text-center px-[5.313rem] py-[5.79px] font-semibold rounded-[4.12px] text-[#F3F3F3] bg-gradient-to-r from-[#006C56] to-[#AFD196]">Dashboard</a>
                <a href="/logout"
                    class="text-center px-[66px] py-[5.79px] font-semibold rounded-[4.12px] text-[#F3F3F3] hover:bg-gradient-to-r from-[#006C56] to-[#AFD196] border-[0.686667px]">Logout</a>
            @else
                <a href="/register"
                    class="text-center px-[66px] py-[5.79px] font-semibold rounded-[4.12px] text-[#F3F3F3] hover:bg-gradient-to-r from-[#006C56] to-[#AFD196] border-[0.686667px]">Sign
                    Up</a>
                <a href="/login"
                    class="text-center px-[5.313rem] py-[5.79px] font-semibold rounded-[4.12px] text-[#F3F3F3] bg-gradient-to-r from-[#006C56] to-[#AFD196]">Login</a>
            @endif
        </div>
    </div>
        ...
        <div x-data="{ IconOpen: false }" @mouseover="IconOpen = true" @mouseleave="IconOpen = false">
            <div class="flex items-center gap-2 cursor-pointer group">
                <a href="/icon"
                    ...
                    Convention</a>
                <x-bi-chevron-down
                    class="w-4 h-4 transition duration-150 group-hover:rotate-180 group-hover:text-blue-200" />
            </div>
    ...
    <div class="flex gap-[0.278rem] xl:gap-[0.625rem]">
        @if (Auth::check())
            <a href="{{ Auth::user()->isAdmin() ? route('admin.') : route('my.') }}"
                class="text-center text-[8.24px] md:text-[10.5px] xl:text-[1rem] px-[0.658rem] md:px-[8px] xl:px-[12px] py-[5.49px] md:py-[8px] rounded-[4.12px] xl:rounded-[6px] bg-gradient-to-r from-[#006C56] to-[#AFD196] text-[#f3f3f3] xl:font-semibold sm">Dashboard</a>
            <a href="/logout"
                class="text-center text-[8.24px] md:text-[10.5px] xl:text-[1rem] px-[0.5rem] md:px-[8px] xl:px-[12px] py-[5.49px] md:py-[8px] border-[0.686667px] xl:border-[1px] border-[#66C1A7] xl:border-[#66C1A7] rounded-[4.12px] xl:rounded-[6px] text-[#F3F3F3] xl:text-[#F3F3F3] font-semibold xl:font-semibold hover:bg-gradient-green-r">Logout</a>
        @else
            <a href="/register"
                class="text-center text-[8.24px] md:text-[10.5px] xl:text-[1rem] px-[0.5rem] md:px-[8px] xl:px-[12px] py-[5.49px] md:py-[8px] border-[0.686667px] xl:border-[1px] border-[#66C1A7] xl:border-[#66C1A7] rounded-[4.12px] xl:rounded-[6px] text-[#F3F3F3] xl:text-[#F3F3F3] font-semibold xl:font-semibold hover:bg-gradient-green-r">Sign
                Up</a>
            <a href="/login"
                class="text-center text-[8.24px] md:text-[10.5px] xl:text-[1rem] px-[0.658rem] md:px-[8px] xl:px-[12px] py-[5.49px] md:py-[8px] rounded-[4.12px] xl:rounded-[6px] bg-gradient-to-r from-[#006C56] to-[#AFD196] text-[#f3f3f3] xl:font-semibold sm">Login</a>
        @endif
    </div>
